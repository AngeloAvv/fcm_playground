...
import 'package:fcm_playground/pages/settings/sheets/theme_selector_value_sheet.dart';
...
class SettingsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(context.l10n?.titleSettings ?? 'titleSettings'),
      ),
        ...
        builder:
            (context, state) => switch (state) {
              ...
              FetchedConfigurationState(:final theme) => SettingsList(
                ...
                sections: [_appSettings(context, theme)],
              ),
  ...
  AbstractSettingsSection _appSettings(BuildContext context, ThemeMode theme) =>
      SettingsSection(
        title: Text(context.l10n?.settingsApp ?? 'settingsApp'),
        tiles: [_themeSection(context, theme)],
      );
      ...
      SettingsTile(
        ...
        onPressed:
            (context) => showModalBottomSheet(
              context: context,
              builder:
                  (context) => ThemeSelectorValueSheet(
                    theme,
                    onChanged:
                        (theme) =>
                            theme?.let((theme) => _setTheme(context, theme)),
                  ),
            ),
      );
